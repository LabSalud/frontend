...
export function AnalysisAccordionView() {
  ...
  const debouncedProtocolSearch = useDebounce(protocolSearchTerm, 300)
  ...
  useEffect(() => {
    localStorage.setItem(RESULTS_ANALYSIS_STATUS_FILTER_KEY, JSON.stringify(statusFilter))
  }, [statusFilter])
  ...
  const fetchResultsByAnalysis = useCallback(
    async (analysisId: number, isInitial = true, silent = false) => {
      if (isInitial) {
        if (!silent) {
      ...
      try {
        ...
        if (isInitial) {
          ...
          if (debouncedProtocolSearch.trim()) {
            params.append("search", debouncedProtocolSearch.trim())
          }
          url += `?${params.toString()}`
        }
    ...
    [apiRequest, showError, nextUrl, statusFilter, protocolsWithResults, debouncedProtocolSearch],
  )
  ...
  // Defer formula recalc para evitar lag por keystroke.
      ...
      formulaTimerRef.current = setTimeout(() => {
        ...
        setResultValues((prev) => {
          ...
          for (const pid of protocolsToRecalc) {
            const protocol = protocolsWithResults.find((item) => item.id === pid)
            if (protocol) next = applyFormulaCalculations(protocol.results, next)
          }
          return next
        })
      }, 250)
  ...
  // Cambio de filtro de estado o search: refetch silent (mantiene lista visible).
  const lastFilteredAnalysisRef = useRef<number | null>(null)
  useEffect(() => {
    if (selectedAnalysis) {
      const silent = lastFilteredAnalysisRef.current === selectedAnalysis.id
      lastFilteredAnalysisRef.current = selectedAnalysis.id
      fetchResultsByAnalysis(selectedAnalysis.id, true, silent)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [statusFilter, debouncedProtocolSearch])
  ...
  // Filtros aplicados server-side (status + search) en fetchResultsByAnalysis.
  // Acá solo pasa la lista tal cual del backend, ya filtrada.
  const filteredProtocols = protocolsWithResults
